...
const activity = {
  updateChatWithProfileReveal: (chat_id) => {
    return models.chat.getById(chat_id)
      .then(chat => {
        ...
        iChat.profile_revealed = PROFILE_REVEAL_STATUS.PUBLIC;
    ...
    return models.user.getById(user_id)
      .then(async sender => {
        ...
        const iNotification = new INotification({
          sender_id: user_id,
          receiver_id,
          type: NOTI_TYPES.chatRevealAccept,
  ...
  pushNotification: async ({ notification }) => {
    ...
    const title = {
      EN: 'Profile Reveal Request Accepted',
      RO: 'Profilul dezvăluie cererea acceptată',
    };
    const body = {
      EN: `${senderName.EN} accepted to reveal profiles of each other.`,
      RO: `${senderName.RO} a acceptat să dezvăluie profiluri între ele.`,
    };
  ...
  respond2Sender: async (socket, { chat, user_id }) => {
    ...
    return models.user.getById(user_id)
      .then(async user => {
        ...
        const partner_id = iChat.user_ids[1 - iChat.user_ids.indexOf(user_id)];
        const partner = await models.user.getById(partner_id);
        const iPartner = new IUser(partner);

        if (iUser.socket_id) {
          socket.emit(CONSTS.SKT_PROFILE_REVEAL_ACCEPT, {
            status: true,
            chat: iChat.toJSON(),
            user: iPartner.asNormal(),
          });
        }
      });
  },
    ...
    return Promise.all([
      ...
      .then(([sender, receiver]) => {
        ...
        if (iReceiver.socket_id) {
          io.to(iReceiver.socket_id).emit(CONSTS.SKT_PROFILE_REVEAL_ACCEPTED, {
            chat: iChat.toObject(),
            user: iSender.asNormal(),
          });
        }
      })
  },
