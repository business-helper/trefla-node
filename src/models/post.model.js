...
class Post extends IPost {
  ...
  output(level = 'OTHERS', keys = []) {
    ...
    data.post_user_id = data.user_id;
    var keys2Delete = ['user_id', 'create_time', 'update_time', 'city'];
...
Post.pagination = async ({
  ...
  user_id ? where.push(`user_id=${user_id}`) : null;
  guest_contacts.length > 0
    ? where.push(`(isGuest = 0 OR (isGuest = 1 AND user_id NOT IN (${guest_contacts.join(',')})))`)
    : 0;

  let zones = [default_zone, location_area].filter((zone) => zone).map((zone) => `'${zone}'`);
...
Post.simplePagination = async ({ limit, page, type = null, user_id = null, sort: { field, desc }, keyword = '' }) => {
  ...
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT *, (like_1_num + like_2_num + like_3_num + like_4_num + like_5_num + like_6_num) as likes 
      FROM posts ${strWhere}
      ORDER BY ${field || 'id'} ${desc ? 'DESC' : 'ASC'} LIMIT ? OFFSET ?`,
      [limit, offset],
      (err, res) => {
        err ? reject(err) : resolve(res);
      }
...
Post.last7DayPosts = (start_time) => {
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT id, feed, create_time, update_time FROM posts WHERE create_time > ?`,
      [start_time],
      (err, res) => {
        err ? reject(err) : resolve(res);
      }
...
Post.output = (post) => {
  ...
  post.photos = JSONParser(post.photos);
  return post;
};
