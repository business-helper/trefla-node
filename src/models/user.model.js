...
User.duplicatedByEmailSocial = (email, login_mode = LOGIN_MODE.NORMAL) => {
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT * FROM ${table} WHERE email=? AND login_mode=? LIMIT 1`,
      [email, login_mode],
      (err, res) => {
        err ? reject(err) : resolve(res[0]);
      }
...
User.getBySocialPass = ({ platform, pass }) => {
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT * FROM ${table} WHERE JSON_EXTRACT(social_pass, "$.${platform}")=?`,
      [pass],
      (err, res) => {
        err ? reject(err) : resolve(res[0]);
      }
...
User.pagination = ({
  ...
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT * FROM users ${strWhere} ORDER BY ${orderBy} ${strLimit}`,
      [offset],
      (err, res) => {
        err ? reject(err) : resolve(res);
      }
...
User.numberOfUsers = ({
  ...
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT COUNT(id) as total FROM users ${strWhere}`,
      [],
      (err, res) => {
...
User.cardPagination = ({ page, limit }) => {
  ...
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT * FROM users WHERE ${strWhere} ORDER BY id DESC ${strLimit}`,
      [],
      (err, res) => {
        err ? reject(err) : resolve(res);
      }
...
User.numberOfCard = () => {
  ...
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT COUNT(id) as total FROM users WHERE ${strWhere}`,
      [],
      (err, res) => {
        err ? reject(err) : resolve(res[0].total);
      }
...
User.output = (user, mode = "NORMAL") => {
  if (!user) return null;
  try {
    if (typeof user.location_array === "string")
