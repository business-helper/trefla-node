...
commentRouters.get("/:id", async (req, res) => {
  ...
  validator.addPostRule(async (provider) =>
    Promise.all([Comment.getById(provider.inputs.id)]).then(([commentById]) => {
      if (!commentById) {
        provider.error(
          "id",
          "custom",
...
commentRouters.post("/:id/like", async (req, res) => {
  ...
  validator.addPostRule(async (provider) =>
    Promise.all([
      ...
      if (!comment) {
        provider.error("id", "custom", `Target comment does not exist!`);
      }
      if (!user) {
...
commentRouters.post("/", async (req, res) => {
  ...
  validator.addPostRule(async (provider) =>
    Promise.all([
      provider.inputs.type === "POST"
        ? Post.getById(provider.inputs.target_id)
        : Comment.getById(provider.inputs.target_id),
    ]).then(([target]) => {
      if (!target) {
        provider.error(
          "target_id",
