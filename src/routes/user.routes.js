...
routes.route('/test').post(async (req, res) => {
...
routes.route('/block/:id').post(async (req, res) => {
  ...
  validator.addPostRule((provider) => {
    return models.user.getById(provider.inputs.id).then((user) => {
...
routes.route('/unblock/:id').post(async (req, res) => {
  const { uid } = getTokenInfo(req);
  const validator = new Validator(
    {
      id: req.params.id,
  ...
  validator.addPostRule((provider) => {
    return Promise.all([models.user.getById(req.params.id), models.user.getById(uid)]).then(([blockee, me]) => {
...
routes.get('/me', async (req, res) => {
  ...
  validator.addPostRule(async (provider) =>
    Promise.all([User.getById(provider.inputs.id)]).then(([userById]) => {
      if (!userById) {
        provider.error('id', 'custom', `User does not exists!`);
      }
    })
  );
...
routes.get('/in-area-users', async (req, res) => {
  req.query.fake = 1;
  const validator = new Validator(req.query, {
    fake: 'required',
  });
...
routes.get('/:id', async (req, res) => {
  ...
  validator.addPostRule(async (provider) =>
    Promise.all([User.getById(provider.inputs.id)]).then(([userById]) => {
      if (!userById) {
        provider.error('id', 'custom', `User with id "${provider.inputs.id}" does not exists!`);
      }
    })
  );
...
routes.post('/ban-reply', async (req, res) => {
 ...
 */
routes.post('/verify/:id', async (req, res) => {
  ...
  validator.addPostRule(async (provider) => {
    Promise.all([User.getById(provider.inputs.id)]).then(([user]) => {
      if (!user) {
 ...
 * @permission admin
 */
routes.post('/unverify/:id', async (req, res) => {
...
routes.post('/transfer-request/reply', async (req, res) => {
  ...
  validator.addPostRule((provider) => {
    return Promise.all([models.notification.getById(provider.inputs.noti_id)]).then(([notification]) => {
...
routes.post('/transfer-request', async (req, res) => {
  const { uid: user_id } = getTokenInfo(req);
  const socketClient = req.app.locals.socketClient;
  ...
  validator.addPostRule((provider) => {
    return models.user.getByCard(provider.inputs.card_number, 1).then(([verifiedUser]) => {
...
routes.post('/verify-request', async (req, res) => {
  return userCtrl
    .createVerifyIdReq(req, res)
    .then((result) => res.json(result))
    .catch((error) => respondValidateError(res, error));
});
...
routes.post('/search', async (req, res) => {
  ...
  return validator
    ...
    .then(() => userCtrl.searchUsersReq(req, res))
    .catch((error) => respondValidateError(res, error));
});

routes.post('/', async (req, res) => {
  userCtrl.pagination(req, res).catch((error) => respondValidateError(res, error));
});

routes.patch('/me', async (req, res) => {
  ...
  if (req.body.email) {
    validator.addPostRule(async (provider) =>
      Promise.all([User.getByEmail(req.body.email), User.getById(id)]).then(([userByEmail, me]) => {
        if (userByEmail && userByEmail.id !== id && userByEmail.login_mode === me.login_mode) {
          provider.error('id', 'custom', `Email already exists with other account!`);
        }
      })
    );
  }
  ...
  if (req.body.user_name) {
    validator.addPostRule(async (provider) =>
      Promise.all([User.getByUserName(req.body.user_name)]).then(([user]) => {
        if (user && user.id !== id) {
          provider.error('id', 'custom', `User name already exists with other account!`);
        }
      })
    );
  }
