    ...
    .then(async (matched) => {
      ...
      if (guess) {
        ...
        if (mGuess.selected_users.length + req.body.target_ids.length > 3)
          throw new Error(`You can selected ${3 - mGuess.selected_users.length} more users in this match!`);
      }
...
routes.route('/preference/add-gender').patch((req, res) => {
  return models.user
    ...
    .then((users) =>
      Promise.all(
        users.map(({ id }) =>
          ctrls.matchProfile.activity.getUserMatchProfile(id).then((matchProfile) => {
