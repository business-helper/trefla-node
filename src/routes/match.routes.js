...
const { GENDER_TYPE } = require("../constants/common.constant");
...
routes.route("/guess/multiple").post((req, res) => {
  ...
  return validator
    ...
    .then(async (matched) => {
      ...
      if (req.body.target_ids.length > 3)
        throw new Error("You can selectd 3 users at max!");
      ...
      if (guess) {
        ...
        if (mGuess.selected_users.length >= 3)
          throw new Error("You already selected the maximum users!");
        if (mGuess.selected_users.length + req.body.target_ids.length > 3)
          throw new Error(
            `You can selected ${
              3 - mGuess.selected_users.length
            } more users in this match!`
          );
      }
      return ctrls.match.guessMultipleUsers(user_id, req.body, socketClient);
    })
    .then(({ guess, matched }) =>
      res.json({ status: true, message: "success", data: guess, matched })
    )
    .catch((error) => respondValidateError(res, error));
});
...
routes.route("/like/:target_id").post((req, res) => {
  ...
  return validator
    ...
    .then(() =>
      ctrls.match.likeUser(
        { my_id: user_id, target_id: Number(req.params.target_id) },
        socketClient
      )
    )
    .then((result) =>
...
routes.route("/profile/me").get((req, res) => {
  const { uid: user_id } = getTokenInfo(req);
  return ctrls.matchProfile
    .getByUserReq(user_id)
...
// needed to filll default match profile for the existing users.
routes.route("/profile/fill-default").patch((req, res) => {
  return models.user
    ...
    .then((users) =>
      Promise.all(
        users.map(({ id }) =>
          ctrls.matchProfile.activity.getUserMatchProfile(id)
        )
      )
    )
    .then((matchProfiles) =>
...
// neeeded to update prefernece to add preference.gender ['Male', 'Female']
routes.route("/preference/add-gender").patch((req, res) => {
  return models.user
    .getAllIds()
    .then((users) =>
      Promise.all(
        users.map(({ id }) =>
          ctrls.matchProfile.activity
            .getUserMatchProfile(id)
            .then((matchProfile) => {
              const mMatchProfile = new models.MatchProfile(matchProfile);
              mMatchProfile.preference.genders = Object.keys(GENDER_TYPE);
    ...
    .then((profiles) =>
      res.json({
        status: true,
        message: `Checked ${profiles.length} users`,
      })
    )
    .catch((error) => res.json({ status: false, message: error.message }));
});
